...
uiGroup *makePage7a(void)
{
	...
	box = newVerticalBox()
	uiGroupSetChild(group, box?.reinterpret())

	area = uiNewArea((uiAreaHandler *) (&handler))
	uiBoxAppend(box, area?.reinterpret(), 1)

	box2 = newHorizontalBox()
	uiBoxAppend(box, box2?.reinterpret(), 0)

	uiBoxAppend(box2, uiNewLabel("Start Angle"?.reinterpret()), 0)
	startAngle = uiNewEntry()
	uiEntryOnChanged(startAngle, entryChanged, NULL)
	uiBoxAppend(box2, startAngle?.reinterpret(), 1)

	box2 = newHorizontalBox()
	uiBoxAppend(box, box2?.reinterpret(), 0)

	uiBoxAppend(box2, uiNewLabel("Sweep"?.reinterpret()), 0)
	sweep = uiNewEntry()
	uiEntryOnChanged(sweep, entryChanged, NULL)
	uiBoxAppend(box2, sweep?.reinterpret(), 1)

	negative = uiNewCheckbox("Negative")
	uiCheckboxOnToggled(negative, checkboxToggled, NULL)
	uiBoxAppend(box, negative?.reinterpret(), 0)

	radians = uiNewCheckbox("Radians")
	uiCheckboxOnToggled(radians, checkboxToggled, NULL)
	uiBoxAppend(box, radians?.reinterpret(), 0)
