...
uiBox *makePage10(void)
{
	...
	uiBoxAppend(hbox, textFontButton?.reinterpret(), 1)

	textColorButton = uiNewColorButton()
	uiColorButtonOnChanged(textColorButton, onColorChanged, NULL)
	uiBoxAppend(hbox, textColorButton?.reinterpret(), 1)
	...
	uiBoxAppend(hbox, noZ?.reinterpret(), 0)
	...
	hbox = newHorizontalBox()
	uiBoxAppend(vbox, hbox?.reinterpret(), 0)
	uiBoxAppend(hbox, uiNewFontButton(?.reinterpret()), 1)
	uiBoxAppend(hbox, uiNewColorButton(?.reinterpret()), 1)
