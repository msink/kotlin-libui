...
{
	if (data != NULL)
		uiControlEnable(area?.reinterpret())
	else
		uiControlDisable(area?.reinterpret())
}
...
uiBox *makePage6(void)
{
	...
	uiBoxAppend(page6, hbox?.reinterpret(), 0)
	...
	uiBoxAppend(hbox, which?.reinterpret(), 0)

	area = uiNewArea((uiAreaHandler *) (&handler))
	uiBoxAppend(page6, area?.reinterpret(), 1)

	hbox = newHorizontalBox()
	uiBoxAppend(page6, hbox?.reinterpret(), 0)

	swallowKeys = uiNewCheckbox("Consider key events handled")
	uiBoxAppend(hbox, swallowKeys?.reinterpret(), 1)
