...
static void handlerDraw(uiAreaHandler *a, uiArea *area, uiAreaDrawParams *dp)
{
	...
	if (entryDouble(textWidth) < 0) { //TODO reuse width
		double ad
...
uiBox *makePage9(void)
{
	...
	uiBoxAppend(hbox, textString?.reinterpret(), 1)

	textFont = uiNewEntry()
	uiEntrySetText(textFont, "Arial")
	uiBoxAppend(hbox, textFont?.reinterpret(), 1)

	textSize = uiNewEntry()
	uiEntrySetText(textSize, "10")
	uiBoxAppend(hbox, textSize?.reinterpret(), 1)
	...
	uiBoxAppend(hbox, textWeight?.reinterpret(), 1)
	...
	uiBoxAppend(hbox, textItalic?.reinterpret(), 1)

	textSmallCaps = uiNewCheckbox("Small Caps")
	uiBoxAppend(hbox, textSmallCaps?.reinterpret(), 1)
	...
	uiBoxAppend(hbox, textStretch?.reinterpret(), 1)

	textWidth = uiNewEntry()
	uiEntrySetText(textWidth, "-1")
	uiBoxAppend(hbox, textWidth?.reinterpret(), 1)
	...
	uiBoxAppend(hbox, textApply?.reinterpret(), 1)

	addLeading = uiNewCheckbox("Add Leading")
	uiCheckboxSetChecked(addLeading, 1)
	uiBoxAppend(hbox, addLeading?.reinterpret(), 0)
	...
	uiBoxAppend(vbox, textArea?.reinterpret(), 1)
