...
//	uiWindowSetChild(w, a?.reinterpret())
{uiBox *b
b=uiNewHorizontalBox()
uiBoxAppend(b,a?.reinterpret(),1)
...
uiBox *makePage15(uiWindow *w)
{
	...
	uiBoxAppend(page15, hbox?.reinterpret(), 0)

	uiBoxAppend(hbox, uiNewLabel("Size"?.reinterpret()), 0)
	width = uiNewSpinbox(INT_MIN, INT_MAX)
	uiBoxAppend(hbox, width?.reinterpret(), 1)
	height = uiNewSpinbox(INT_MIN, INT_MAX)
	uiBoxAppend(hbox, height?.reinterpret(), 1)
	fullscreen = uiNewCheckbox("Fullscreen")
	uiBoxAppend(hbox, fullscreen?.reinterpret(), 0)
	...
	uiBoxAppend(page15, checkbox?.reinterpret(), 0)

	button = uiNewButton("Borderless Resizes")
	uiButtonOnClicked(button, borderWindowOpen, NULL)
	uiBoxAppend(page15, button?.reinterpret(), 0)

	hbox = newHorizontalBox()
	uiBoxAppend(page15, hbox?.reinterpret(), 1)

	uiBoxAppend(hbox, uiNewVerticalSeparator(?.reinterpret()), 0)
